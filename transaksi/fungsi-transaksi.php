...
function ambilPeminjaman($connect, $id_pinjam){
	$sql = "SELECT * FROM peminjaman INNER JOIN anggota ON peminjaman.id_anggota=anggota.id_anggota INNER JOIN detail_pinjam USING(id_pinjam) INNER JOIN buku ON detail_pinjam.id_buku=buku.id_buku where id_pinjam='$id_pinjam'" ;
	$res = mysqli_query($connect, $sql);
